...
import {
  callRepositoryTaskTool,
  repositoryTaskMcpContract,
  repositoryTaskTools,
} from './repository-task-core.mjs';

const PROTOCOL_VERSION = '2025-06-18';
const MESSAGE_LIMIT = 512 * 1024;
...
async function handle(raw) {
  ...
  if (raw.method === 'initialize') {
    ...
    send(response(id, {
      ...
      serverInfo: { name: repositoryTaskMcpContract.serverName, version: repositoryTaskMcpContract.serverVersion },
      instructions: 'Governed EVAVO named repository-task describe/plan/run. Fragment manifests are exact-Git and digest-bound. No caller executable, script, argv, environment, shell or physical path authority is accepted.',
    }));
    return;
  }
  ...
  if (raw.method === 'tools/list') { if (!notification) send(response(id, { tools: repositoryTaskTools })); return; }
  if (raw.method === 'tools/call') {
    ...
    try {
      const result = await callRepositoryTaskTool(params.name, object(params.arguments) ? params.arguments : {});
