...
const TOOLS = Object.freeze([
  ...
  },
  {
    ...
    inputSchema: { type: "object", additionalProperties: false, required: ["action"], properties: { action: { enum: [...TYPED_ACTIONS] } } },
  },
  {
    ...
    inputSchema: { type: "object", additionalProperties: false, required: ["root", "path"], properties: { root: { enum: [...READ_ROOTS] }, path: { type: "string", minLength: 1, maxLength: 4096 }, limit: { type: "integer", minimum: 1, maximum: 1000 } } },
  },
  {
    ...
    inputSchema: { type: "object", additionalProperties: false, required: ["sourceRoot", "sourcePath", "destinationRoot", "destinationPath"], properties: { sourceRoot: { enum: [...READ_ROOTS] }, sourcePath: { type: "string", minLength: 1, maxLength: 4096 }, destinationRoot: { enum: [...WRITE_ROOTS] }, destinationPath: { type: "string", minLength: 1, maxLength: 4096 } } },
  },
  {
    ...
    inputSchema: { type: "object", additionalProperties: false, required: ["commandType", "command"], properties: { commandType: { enum: [...OPERATOR_TYPES] }, command: { type: "string", minLength: 1, maxLength: 32768 }, cwdRoot: { enum: [...READ_ROOTS] }, cwdRelative: { type: "string", minLength: 1, maxLength: 4096 }, timeoutSeconds: { type: "integer", minimum: 1, maximum: 300 } } },
  },
]);
...
async function readJsonFile(path, minBytes, maxBytes) {
...
async function loadOperatorToken() {
  ...
  if ((process.env.EVAVO_LOCAL_AGENT_REST_OPERATOR_EXEC ?? "").trim() !== "1") throw new Error("Local Agent operator execution is not explicitly enabled in this MCP process");
...
async function postJson(path, payload, token, timeoutMs) {
  let response;
  try {
    response = await fetch(`${REST_BASE}${path}`, { method: "POST", headers: { "Content-Type": "application/json", "X-EVAVO-Local-Agent-Token": token }, body: JSON.stringify(payload), signal: AbortSignal.timeout(timeoutMs), cache: "no-store" });
  } catch { throw new Error("Local Agent REST request did not return; execution outcome may be unknown and was not retried"); }
  let body;
  try { body = await response.json(); } catch { throw new Error("Local Agent REST returned invalid JSON; request was not retried"); }
...
async function callTool(name, raw) {
  ...
  if (name === "evavo_local_file_list") {
    const root = String(args.root ?? ""); const path = String(args.path ?? ""); const limit = args.limit === undefined ? 250 : Number(args.limit);
    if (!includes(READ_ROOTS, root) || !path || path.length > 4096 || !Number.isInteger(limit) || limit < 1 || limit > 1000) throw new Error("file-list arguments are invalid");
    const credential = await loadTypedToken();
  ...
  if (name === "evavo_local_file_copy") {
    const sourceRoot = String(args.sourceRoot ?? ""); const destinationRoot = String(args.destinationRoot ?? ""); const sourcePath = String(args.sourcePath ?? ""); const destinationPath = String(args.destinationPath ?? "");
    if (!includes(READ_ROOTS, sourceRoot) || !includes(WRITE_ROOTS, destinationRoot) || !sourcePath || !destinationPath || sourcePath.length > 4096 || destinationPath.length > 4096) throw new Error("file-copy arguments are invalid");
    const credential = await loadTypedToken();
  ...
  if (name === "evavo_local_operator_execute") {
    const commandType = String(args.commandType ?? ""); const command = String(args.command ?? ""); const cwdRoot = String(args.cwdRoot ?? "gitrepos"); const cwdRelative = String(args.cwdRelative ?? "."); const timeoutSeconds = args.timeoutSeconds === undefined ? 120 : Number(args.timeoutSeconds);
    if (!includes(OPERATOR_TYPES, commandType) || !command || command.length > 32768 || !includes(READ_ROOTS, cwdRoot) || !cwdRelative || cwdRelative.length > 4096 || !Number.isInteger(timeoutSeconds) || timeoutSeconds < 1 || timeoutSeconds > 300) throw new Error("operator arguments are invalid");
...
for await (const line of input) {
  ...
  try {
    ...
    else if (request.method === "initialize") write(result(request.id, { protocolVersion: "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "evavo-agent-mcp", version: "1.1.0" } }));
    else if (request.method === "tools/list") write(result(request.id, { tools: TOOLS }));
    else if (request.method === "tools/call") {
      const params = asObject(request.params); const value = await callTool(String(params.name ?? ""), params.arguments);
