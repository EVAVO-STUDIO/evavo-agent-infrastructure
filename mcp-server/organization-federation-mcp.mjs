...
import {
  callOrganizationFederationTool,
  organizationFederationMcpContract,
  organizationFederationTools,
} from './organization-federation-core.mjs';
...
async function handle(raw) {
  ...
  if (raw.method === 'initialize') {
    ...
    send(response(id, {
      ...
      serverInfo: { name: organizationFederationMcpContract.serverName, version: organizationFederationMcpContract.serverVersion },
      instructions: 'Read-only EVAVO organisation federation planning. Domain ownership, team planning, route readiness and handoff validation only. This server never executes, mutates, publishes, approves, sends device control or grants authority.'
    }));
    return;
  }
  ...
  if (raw.method === 'tools/list') { if (!notification) send(response(id, { tools: organizationFederationTools })); return; }
  if (raw.method === 'tools/call') {
    ...
    try {
      const result = await callOrganizationFederationTool(params.name, Object.hasOwn(params, 'arguments') ? params.arguments : {});
