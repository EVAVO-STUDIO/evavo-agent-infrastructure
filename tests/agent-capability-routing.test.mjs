...
test('completed and physical claims require a current correlated receipt', () => {
  const result = plan(
    status({
      ...
      evidence: [
        evidence('device-android-typed-relay', 'physically_verified', {
  ...
  const decision = result.decisions[0];
  assert.equal(decision.claims.mayClaimCompleted, true);
  assert.equal(decision.claims.mayClaimPhysicallyVerified, true);
});
...
test('stale evidence is visible but cannot select or prove a route', () => {
  ...
  assert.equal(
    result.decisions[0].candidates.find((candidate) => candidate.strategyId === 'host-execute-issue-queue').reason,
    'stale-evidence',
  );
});
...
test('explicit unhealthy evidence blocks the capability rather than weakening the route', () => {
  const result = plan(
    status({
      requestedCapabilities: ['host.execute'],
      evidence: [
        evidence('host-execute-typed-relay', 'transport_online', {
          healthy: false,
...
test('unknown capabilities are denied with no inferred generic shell fallback', () => {
  const result = plan(
    status({
      requestedCapabilities: ['host.raw-shell'],
      evidence: [],
    }),
  );
  assert.equal(result.decisions[0].status, 'blocked');
...
test('effectful routes cannot split authority across repositories', () => {
  ...
  assert.throws(
    () => validateRoutingConfig(tampered),
    /EVAVO_AGENT_ROUTING_EFFECT_AUTHORITY_SPLIT/u,
  );
});
...
test('transport ingress diversity cannot be overstated', () => {
  ...
  assert.throws(
    () => validateRoutingConfig(tampered),
    /EVAVO_AGENT_ROUTING_FAILURE_DOMAIN_COUNT/u,
  );
});
...
test('raw shell and paid hosted execution cannot be introduced through config', () => {
  ...
  assert.throws(
    () => validateRoutingConfig(rawShell),
    /EVAVO_AGENT_ROUTING_TRANSPORT_ARBITRARY_SHELL/u,
  );

  const hosted = structuredClone(configDocument);
  hosted.transports['cloudflare-typed-relay'].failureDomain = 'github-actions';
  assert.throws(
    () => validateRoutingConfig(hosted),
    /EVAVO_AGENT_ROUTING_HOSTED_EXECUTION/u,
  );
  ...
  assert.throws(
    () => validateRoutingConfig(secondExecutor),
    /EVAVO_AGENT_ROUTING_PHYSICAL_EXECUTOR/u,
  );
});
...
test('strict JSON rejects duplicate and prototype-polluting properties', () => {
  assert.throws(
    ...
    /EVAVO_AGENT_ROUTING_JSON_DUPLICATE_KEY/u,
  );
  assert.throws(
    () => parseStrictJson('{"__proto__":{}}'),
    /EVAVO_AGENT_ROUTING_JSON_PROHIBITED_KEY/u,
  );
});
