...
const WRITE_OPERATIONS = new Set([
  "project.create", "project.update", "deployment.deploy",
  "domain.add", "domain.update", "domain.verify", "domain.remove",
]);
...
async function execute(env: Env, input: unknown, allowWrite: boolean): Promise<JsonObject> {
  ...
  switch (operation) {
    ...
    case "project.create": {
      ...
      const git = request.gitRepository === undefined ? null : object(request.gitRepository);
      let gitRepository: JsonObject | undefined;
      if (git) {
      ...
      data = await provider(env, "POST", "/v11/projects", { name, ...settings, ...(gitRepository ? { gitRepository } : {}) });
      break;
    }
    ...
    case "domain.add": {
      const body: JsonObject = { name: domain(request.domain) };
      ...
      if (request.redirectStatusCode !== undefined) {
        const code = Number(request.redirectStatusCode); if (![301,302,307,308].includes(code)) throw new Error("redirect-status-invalid"); body.redirectStatusCode = code;
      }
      data = await provider(env, "POST", `/v10/projects/${encode(project(request.project))}/domains`, body); break;
    }
    case "domain.update": {
      ...
      data = await provider(env, "PATCH", `/v9/projects/${encode(project(request.project))}/domains/${encode(domain(request.domain))}`, body); break;
    }
...
function makeServer(env: Env): McpServer {
  const server = new McpServer({ name: "EVAVO Vercel Provider Control", version: "1.0.0" });
  ...
  server.registerTool("evavo_vercel_provider_control", {
    description: "Plan or execute governed cloud-side Vercel project, Git deployment, and custom-domain operations. Writes require execute=true plus a reason; destructive operations additionally require allowDestructive=true.",
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/health" && request.method === "GET") {
      return new Response(JSON.stringify({ ok: true, service: "evavo-vercel-provider-relay", workstationRequired: false, providerCredentialConfigured: Boolean(env.VERCEL_TOKEN), controlCredentialConfigured: Boolean(env.CONTROL_TOKEN), credentialValuesReturned: false }), { headers: { "content-type": "application/json", "cache-control": "no-store" } });
    }
    if (url.pathname === "/mcp") {
