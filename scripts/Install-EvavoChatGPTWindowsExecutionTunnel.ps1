...
param(
    ...
    [string]$TunnelName = 'EVAVO Windows Execution',
    [string]$TunnelId = '',
    [string]$WorkspaceId = '',
...
if ($ContractReceipt.ok -ne $true -or $ContractReceipt.arbitraryCommandTextAccepted -ne $true -or $ContractReceipt.currentWindowsUserAuthority -ne $true) { throw 'EVAVO_WINDOWS_EXECUTION_TUNNEL_CONTRACT_NOT_ACCEPTED' }
...
$Launcher = Join-Path $Bundle 'run-windows-chat-execution-mcp.ps1'
...
`$env:EVAVO_WINDOWS_CHAT_ALLOWED_ROOTS='C:\GitRepos;%LOCALAPPDATA%\EVAVO;%USERPROFILE%\Downloads'
`$env:EVAVO_LOCAL_STORAGE_ROOT='C:\GitRepos\evavo-local-storage'
`$env:EVAVO_SERVICE_CONTROL_PLANE_ROOT='C:\GitRepos\evavo-service-control-plane'
...
[IO.File]::WriteAllText($ManifestPath,([ordered]@{
    schemaVersion=1
    kind='evavo-chatgpt-windows-execution-bundle-v1'
...
if ([string]::IsNullOrWhiteSpace($TunnelId)) {
    ...
    $Args=@('admin','tunnels','create','--name',$TunnelName,'--description','Effectful EVAVO Windows execution MCP: PowerShell, CMD, Bash and Python through physically accepted loopback executor.','--json')
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger @($Logon,$Periodic) -Principal $Principal -Settings $Settings -Description 'Outbound-only OpenAI Secure MCP Tunnel for effectful EVAVO Windows shell execution.' -Force|Out-Null
...
[ordered]@{
    schemaVersion=1
    kind='evavo-chatgpt-windows-execution-tunnel-installation-v1'
    ok=$true
