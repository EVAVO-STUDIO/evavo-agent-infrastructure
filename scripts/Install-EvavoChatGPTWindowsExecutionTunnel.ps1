...
if (-not (Test-Path -LiteralPath $WScriptExe -PathType Leaf)) { throw 'EVAVO_WINDOWS_EXECUTION_TUNNEL_WSCRIPT_REQUIRED' }
...
$ContractRaw = (& $PowerShellExe -NoLogo -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File $Contract | Out-String).Trim()
...
$LegacyLauncher = Join-Path $Bundle 'run-windows-chat-execution-compatibility-mcp.ps1'
$TaskLauncher = Join-Path $Bundle 'run-windows-execution-tunnel-hidden.vbs'
...
# Retain the historical PowerShell launcher as a non-authoritative compatibility
# artifact only. The tunnel profile now invokes Node directly, eliminating an
# unnecessary console-capable process from every MCP child start.
$LegacyLauncherBody = @"
Set-StrictMode -Version Latest
`$ErrorActionPreference='Stop'
...
exit `$LASTEXITCODE
"@
[IO.File]::WriteAllText($LegacyLauncher,$LegacyLauncherBody,[Text.UTF8Encoding]::new($false))
$Tokens=$null;$Errors=$null;[Management.Automation.Language.Parser]::ParseFile($LegacyLauncher,[ref]$Tokens,[ref]$Errors)|Out-Null
if (@($Errors).Count -gt 0) { throw 'EVAVO_WINDOWS_EXECUTION_TUNNEL_LEGACY_LAUNCHER_PARSE_FAILED' }
$LegacyLauncherHash = (Get-FileHash -LiteralPath $LegacyLauncher -Algorithm SHA256).Hash.ToLowerInvariant()
...
$McpCommand = ('"{0}" "{1}"' -f $NodeExe,$InstalledMcp)
...
$TunnelPayload = ('"{0}" {1}' -f $TunnelExe,$TaskArguments)
...
if ($TaskLauncherItem.PSIsContainer -or (($TaskLauncherItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0)) { throw 'EVAVO_WINDOWS_EXECUTION_TUNNEL_TASK_LAUNCHER_UNSAFE' }
...
[IO.File]::WriteAllText($ManifestPath,([ordered]@{
    ...
    launcherSha256=$LegacyLauncherHash
    legacyPowerShellLauncherAuthoritative=$false
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger @($Logon,$Periodic) -Principal $Principal -Settings $Settings -Description 'Outbound-only read-only OpenAI Secure MCP compatibility tunnel. Windowless scheduled host; effectful Windows execution is owned by Local Compute typed routes.' -Force|Out-Null
