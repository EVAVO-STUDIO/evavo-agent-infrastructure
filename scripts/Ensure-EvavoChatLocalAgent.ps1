...
if (-not (Test-Path -LiteralPath $AcceptanceScript -PathType Leaf)) { throw 'Canonical Local Agent MCP acceptance script is missing.' }
if (-not (Test-Path -LiteralPath $NodeManager -PathType Leaf)) { throw "EVAVO Local Storage node manager was not found at $NodeManager" }
...
if (-not $Initial.ok -and -not $NoRepair) {
    ...
    if (-not $Repair.ok) {
        $Receipt = [ordered]@{
            schemaVersion=1;kind='evavo-chat-local-agent-readiness-v1';ok=$false;repaired=$false;reason='repair-failed';startedAt=$StartedAt.ToString('O');completedAt=[DateTimeOffset]::UtcNow.ToString('O')
            agentInfrastructureRoot=$AgentInfraRoot;localStorageRoot=$LocalStorageRoot;steps=@($Steps);initialAcceptanceOutput=$Initial.output;diagnosisOutput=$Diagnosis.output;repairOutput=$Repair.output
            permanentDeleteAuthorityAdded=$false;operatorExecutionRequested=[bool]$IncludeOperatorExecution;childPowerShellHidden=$true;focusStealAllowed=$false;desktopCommanderRequired=$false
        }
        if ($Json) { $Receipt | ConvertTo-Json -Depth 10 } else { $Receipt }; exit 1
    }
    $Restart = Invoke-NodeManager -Action restart
    $Steps.Add([pscustomobject]@{ step='restart'; ok=$Restart.ok; exitCode=$Restart.exitCode })
    if (-not $Restart.ok) { throw 'Local Storage repair completed but autonomous-node restart failed.' }
    $deadline = [DateTimeOffset]::UtcNow.AddSeconds(45)
    do {
        Start-Sleep -Milliseconds 750
        $socketReady = $false
        try { $client=[Net.Sockets.TcpClient]::new();try{$task=$client.ConnectAsync('127.0.0.1',4329);if($task.Wait(800)-and$client.Connected){$socketReady=$true}}finally{$client.Dispose()} } catch { $socketReady=$false }
...
$Receipt = [ordered]@{
    schemaVersion=1;kind='evavo-chat-local-agent-readiness-v1';ok=[bool]($Final.ok -and $Status.ok);repaired=[bool]$Repaired;repairAttempted=[bool](-not $Initial.ok -and -not $NoRepair);noRepairRequested=[bool]$NoRepair
    operatorExecutionRequested=[bool]$IncludeOperatorExecution;workstationAcceptanceRequested=[bool]$IncludeWorkstationAcceptance;loopbackRest='http://127.0.0.1:4329';canonicalMcp='mcp-server/local-agent-mcp.mjs';localStorageNodeManager='scripts/manage-autonomous-node.ps1'
    startedAt=$StartedAt.ToString('O');completedAt=[DateTimeOffset]::UtcNow.ToString('O');steps=@($Steps);acceptanceOutput=$Final.output;nodeStatusOutput=$Status.output;automaticRetryCount=$(if($Initial.ok){0}else{1})
    permanentDeleteAuthorityAdded=$false;childPowerShellHidden=$true;focusStealAllowed=$false;desktopCommanderRequired=$false
}
