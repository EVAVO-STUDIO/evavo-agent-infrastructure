...
const [workPath, routePath, capabilityPath, workerId] = process.argv.slice(2);
if (!workPath || !routePath || !capabilityPath || !workerId) {
  console.error("Usage: node scripts/compile-codex-worker-dispatch.mjs <leased-work.json> <route-plan.json> <codex-capability.json> <worker-id>");
  process.exit(2);
}
...
if (!capability.capabilities?.jsonFlag || !capability.capabilities?.modelFlag) errors.push("Codex capability receipt lacks required flags.");
...
console.log(JSON.stringify({
  ...
  argv,
  stdinPrompt: prompt,
  workingDirectoryPolicy: "exact-admitted-repository-root",
  shell: false,
  ...
  truthBoundary: "This plan compiles a bounded Codex Exec invocation but does not execute it. Runtime dispatch must recheck repository identity, exact HEAD, worktree policy, lease validity, Codex capability freshness and route capacity."
}, null, 2));
