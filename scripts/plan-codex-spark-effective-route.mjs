...
if (!workItemInput || !effectiveCapacityInput) {
  console.error("Usage: node scripts/plan-codex-spark-effective-route.mjs <ready-work-item.json> <effective-capacity-status.json>");
  process.exit(2);
}
...
if (!spark) errors.push("Effective-capacity status lacks the Spark route.");
  ...
  if (!Array.isArray(spark.admittedWorkerClasses) || !spark.admittedWorkerClasses.includes("test-generation")) {
    errors.push("Effective Spark route does not admit Test Builder.");
  }
  const unadmitted = (spark.admittedWorkerClasses ?? []).filter((entry) => entry !== "test-generation");
  if (unadmitted.length) errors.push(`Effective Spark route contains unapproved worker classes: ${unadmitted.join(", ")}.`);
  if (spark.maximumConcurrency !== 1 || spark.maximumAutomaticConcurrency !== 1) errors.push("Effective Spark route must remain at concurrency one.");
  for (const field of [
