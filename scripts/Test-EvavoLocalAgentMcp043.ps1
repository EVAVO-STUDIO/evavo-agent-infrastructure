...
try{
    ...
    $OperatorProof=@{};if($IncludeOperatorExecution){foreach($Case in $OperatorCases){$Payload=Read-ToolPayload ([int]$Case.id);$Stdout=[string]$Payload.stdout;$Passed=[bool]($Payload.ok-eq$true-and$Payload.operatorAuthority-eq$true-and$Payload.shellParameterUsed-eq$false-and$Stdout.Contains([string]$Case.marker));$OperatorProof[[string]$Case.type]=$Passed;if(-not$Passed){throw"Local Agent MCP operator probe failed: $($Case.type)"}}}
    ...
    $Receipt=[ordered]@{
        ...
        fileWriteCreateOnlyProven=$true;fileReadHashAndContentProven=$true;fileListNonRecursiveProven=$true;fileCopyDestinationHashProven=$true;gitReposNormalWriteDeniedByContract=$true;doctorOwnedTemporaryCleanupOnly=$true;temporaryAcceptanceFilesRemoved=$DoctorOwnedCleanupPerformed;
        ...
        powershellProven=if($IncludeOperatorExecution){[bool]$OperatorProof.powershell}else{$null};pythonProven=if($IncludeOperatorExecution){[bool]$OperatorProof.python}else{$null};bashProven=if($IncludeOperatorExecution){[bool]$OperatorProof.bash}else{$null};cmdProven=if($IncludeOperatorExecution){[bool]$OperatorProof.cmd}else{$null};
        operatorExecutionRequiresExplicitLocalEnable=$true;externalNetworkRequiredForMcp=$false;githubActionsRequired=$false;credentialValuesReturned=$false;physicalPathsReturned=$false;forcePush=$false;permanentDeleteAuthorityAdded=$false
    }
