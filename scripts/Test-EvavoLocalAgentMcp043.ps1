...
if (($NodeItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'Local Agent MCP Node executable may not be a reparse point.' }
...
try {
    $FileHash = ([BitConverter]::ToString($Hasher.ComputeHash($FileBytes))).Replace('-','').ToLowerInvariant()
} finally {
    $Hasher.Dispose()
...
foreach ($Path in @($WorkRoot,$ReceiptRoot)) {
    ...
    if (-not $Item.PSIsContainer -or ($Item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'MCP acceptance state roots must be canonical non-reparse directories.' }
}
...
try {
    ...
    if ($Process.ExitCode -ne 0) { throw "Local Agent MCP runtime exited $($Process.ExitCode)." }
    ...
    $Write = Read-ToolPayload 20
    if ($Write.ok -ne $true -or [string]$Write.kind -ne 'evavo-local-agent-rest-file-write-v1' -or $Write.createOnly -ne $true -or $Write.gitReposWriteAllowed -ne $false -or [string]$Write.sha256 -ne $FileHash) { throw 'MCP create-only file write proof failed.' }
    $Read = Read-ToolPayload 21
    if ($Read.ok -ne $true -or [string]$Read.kind -ne 'evavo-local-agent-rest-file-read-v1' -or [string]$Read.sha256 -ne $FileHash) { throw 'MCP file read proof failed.' }
    ...
    $Copy = Read-ToolPayload 22
    if ($Copy.ok -ne $true -or [string]$Copy.kind -ne 'evavo-local-agent-rest-file-copy-v1' -or $Copy.createOnly -ne $true -or $Copy.destinationHashVerified -ne $true -or $Copy.gitReposWriteAllowed -ne $false -or [string]$Copy.sha256 -ne $FileHash) { throw 'MCP verified file copy proof failed.' }
    $Listing = Read-ToolPayload 23
    if ($Listing.ok -ne $true -or [string]$Listing.kind -ne 'evavo-local-agent-rest-file-list-v1' -or $Listing.recursive -ne $false) { throw 'MCP file list proof failed.' }
    $Names = @($Listing.entries | ForEach-Object { [string]$_.name })
    if ($Names -notcontains 'source.txt' -or $Names -notcontains 'copy.txt') { throw 'MCP file list did not observe doctor-owned artifacts.' }
    ...
    if ($IncludeOperatorExecution) {
        foreach ($Case in $OperatorCases) {
            ...
            if ([string]$Case.type -eq 'pwsh') {
                $Passed = [bool]($Passed -and [string]$Payload.requestedCommandType -eq 'pwsh' -and [string]$Payload.operatorAdapterMode -eq 'encoded-pwsh-via-fixed-powershell-launcher' -and $Payload.pwshUserCodeEncoded -eq $true)
            }
    ...
    if (Test-Path -LiteralPath $DoctorOwnedPhysicalRoot) { Remove-Item -LiteralPath $DoctorOwnedPhysicalRoot -Recurse -Force -ErrorAction Stop }
    $DoctorOwnedCleanupPerformed = $true
