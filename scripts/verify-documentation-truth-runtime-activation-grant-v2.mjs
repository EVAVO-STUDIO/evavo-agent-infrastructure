...
function parseArguments(values) {
  ...
  for (let index = 0; index < values.length; index += 2) {
    ...
    if (!key?.startsWith("--") || !value || value.startsWith("--") || options.has(key)) {
      throw new Error(
        "Usage: node scripts/verify-documentation-truth-runtime-activation-grant-v2.mjs --envelope <json> --trust-anchor <json> --request <json> --now <ISO-8601> --consumed-uses <integer>",
      );
    }
...
export async function verifyDocumentationTruthRuntimeActivationGrantV2Files({
  envelopePath,
  trustAnchorPath,
  requestPath,
  now,
  ...
  const verified = await verifyDocumentationTruthRuntimeActivationGrantV2({
  ...
  return {
    ...
    verificationSha256: sha256(
      Buffer.from(canonicalDocumentationTruthRuntimeRequestJson(body), "utf8"),
    ),
  };
}
...
if (directInvocation) {
  try {
    ...
    process.stderr.write(`${JSON.stringify({
      ...
      accepted: false,
      exactRequestIdentityVerified: false,
