...
$Servers = @(
    ...
    [ordered]@{ file='mcp-server\glasses-android-mcp.mjs'; name='evavo-glasses-android-mcp'; version='1.2.0'; tools=@('evavo_glasses_android_preflight','evavo_glasses_android_doctor','evavo_glasses_android_build','evavo_glasses_android_test_device','evavo_glasses_android_build_and_test','evavo_glasses_android_acceptance_status'); readOnlyOnly=$false },
...
foreach ($Server in $Servers) {
    ...
    try {
        ...
        if ($Server.name -eq 'evavo-glasses-android-mcp') {
            $Preflight = @($List.result.tools | Where-Object { $_.name -eq 'evavo_glasses_android_preflight' }) | Select-Object -First 1
            if ($null -eq $Preflight -or $Preflight.annotations.readOnlyHint -ne $true -or $Preflight.annotations.destructiveHint -ne $false) {
                throw 'EVAVO Glasses Android preflight is not explicitly read-only.'
            }
        }
